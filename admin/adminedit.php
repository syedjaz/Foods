...
if(isset($_POST['updateadmin'])){
    ...
    if(isset($_FILES['aimage'])){
        $errors = array();
        $filename = rand(0,10).$_FILES['aimage']['name'];
        $tmpname = $_FILES['aimage']['tmp_name'];
        $size = $_FILES['aimage']['size'];
    ...
    $update_sql = "UPDATE admin SET a_name = '{$aname}' , a_email = '{$aemail}', img = '{$filename}' WHERE a_id = '{$id}'";
 
    }
    else{
        $aname = $_POST['aname'];
        $aemail = $_POST['aemail'];
        $update_sql = "UPDATE admin SET a_name = '{$aname}' , a_email = '{$aemail}' WHERE a_id = '{$id}'";
 
      
    }
    $res = mysqli_query($con,$update_sql);
    if($res){
      echo "<script>window.location.href = 'http://localhost:82/ebook/admin/dashboard.php';</script>";
    }
}
$read_book = "SELECT * FROM admin WHERE a_id = '{$id}'";
...
<span class="text-small align-middle">Update Admin Details</span>
</a>
<h1 class="mb-0 pb-0 display-4" id="title">Admin Details</h1>
</div>
</div>
...
<input type="text" class="form-control" name="aname" value="<?php echo $data['a_name']?>">
</div>

<div class="mb-3">
<label class="form-label">Email</label>
<input type="text" class="form-control" name="aemail" value="<?php echo $data['a_email']?>">
</div>
<div class="mb-3">
...
<input type="submit" name="updateadmin" value="Update Admin Details" class="btn btn-primary btn-outline-light">
</div>
</div>
<?php
    }
