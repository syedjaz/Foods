...
if(isset($_POST['updatebook'])){
    ...
    $update_sql = "UPDATE books SET b_name = '{$bname}' , b_price = '{$bprice}',b_mrp = '{$bmrp}',b_image = '{$filename}',cate_id = '{$bcat}' WHERE b_id = '{$id}'";
        ...
        $bmrp = $_POST['bmrp'];
        $bcat = $_POST['bcat'];
        $update_sql = "UPDATE books SET b_name = '{$bname}' , b_price = '{$bprice}',b_mrp = '{$bmrp}',cate_id = '{$bcat}' WHERE b_id = '{$id}'";
 
      
    }
    $res = mysqli_query($con,$update_sql);
    if($res){
      echo "<script>window.location.href = 'http://localhost:82/ebook/admin/view_books.php';</script>";
    }
}
$read_book = "SELECT * FROM books WHERE b_id = '{$id}'";
...
<span class="text-small align-middle">Update Book Details</span>
</a>
<h1 class="mb-0 pb-0 display-4" id="title">Book Details</h1>
</div>
</div>
...
<input type="text" class="form-control" name="bname" value="<?php echo $data['b_name']?>">
</div>

<div class="mb-3">
<label class="form-label">Price</label>
<input type="text" class="form-control" name="bprice" value="<?php echo $data['b_price']?>">
</div>

<div class="mb-3">
<label class="form-label">MRP</label>
<input type="text" class="form-control" name="bmrp" value="<?php echo $data['b_mrp']?>">
</div>
<div class="mb-3">
...
<select class="form-control" name="bcat">
    ...
    $result = mysqli_query($con,$fetch_cat);
    $total_rows_cat = mysqli_num_rows($result);
    if($total_rows_cat!=0){
        while($data_cat = mysqli_fetch_assoc($result)){
    if($data_cat['cate_id']==$data['cat_id'])
    {
        $select = "selected";
    }
    else{
    ...
    ?>
<option <?php echo $select?> value="<?php echo $data_cat['cat_id']?>"><?php echo $data_cat['cat_name']?></option>    
<?php
        }
    }
?>
...
<input type="submit" name="updatebook" value="Update Book Details" class="btn btn-primary btn-outline-light">
</div>
</div>
<?php
    }
