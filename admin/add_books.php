...
if(isset($_POST['addbook'])){
    ...
    if(isset($_FILES['bimage'])){
        $errors = array();
        $filename = rand(0,10).$_FILES['bimage']['name'];
        ...
        move_uploaded_file($tmpname,'media/'.$filename);
            ...
            echo "<script>window.location.href = 'http://localhost:82/ebook/admin/add_books.php';</script>";
            die();
    ...
    $ins_pro = "INSERT INTO books(b_id,b_name,b_price,b_mrp,b_image,cate_id) VALUES('{$id}','{$name}','{$price}','{$mrp}','{$filename}','{$bcat}')";

    $res = mysqli_query($con,$ins_pro);
    if($res){
...
<span class="text-small align-middle">Books</span>
</a>
...
<div class="col-xl-12">
<div class="mb-5">
<h2 class="small-title">Book Info</h2>
<div class="card">
<div class="card-body">
<form method="POST" enctype="multipart/form-data">
<div class="mb-3">
<label class="form-label">Book ID</label>
<input type="text" name="bid" class="form-control" value="<?php function generatePIN($digits = 5){ $i = 0; //counter
...
//$appid_end = "AKS".
$pin = generatePIN();
echo $pin;?>" id="name-form7-15" readonly>
</div>
<div class="mb-3">
...
<select class="select-single-no-search" name="bcat">
<option disabled>Select</option>
                                        <?php
                                        
                                        $fetch_sql = "SELECT * FROM categories ORDER BY cat_id";
...
<input type="text" class="form-control" name="bmrp">
</div>
<div class="mb-3">
...
<input type="submit" name="addbook" value="Add Book Info" class="btn btn-primary btn-outline-light">
