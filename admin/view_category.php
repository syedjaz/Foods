...
if(isset($_GET['type']) && $_GET['type']!=''){
    ...
    $operation = get_safe_value($con,$_GET['operation']);
     ...
     $update = "UPDATE categories SET cat_status = '{$status}' WHERE cat_id = '{$id}'";
     $result = mysqli_query($con,$update);
     if($result){
     echo "<script>window.location.href = 'http://localhost:82/ebook/admin/view_category.php';</script>";
     }
    }
 }
?>
...
<a href="add_category.php"><i data-cs-icon="plus"></i>
...
<?php
    ...
    <div class="card mb-2">
        <div class="row g-0 h-100 sh-lg-9 position-relative">
            ...
            <div class="lh-1 text-alternate"><?php echo $data['cat_id']?></div>
        </div>
        <div class="col-12 col-lg-4 d-flex flex-column pe-1 mb-2 mb-lg-0 justify-content-center order-3">
            <div class="lh-1 text-alternate"><?php echo $data['cat_name']?></div>
        </div>
        ...
        <?php 
                            ...
                            {
                                echo '<span class="badge" style="background-color:#2cbfee;"><a class="btn btn-sm" href="?type=status&operation=deactive&id='.$data['cat_id'].'">Active</a></span>';
                            }
                            else{
                                echo '<span class="badge" style="background-color:#c1cbce;"><a class="btn btn-sm" href="?type=status&operation=active&id='.$data['cat_id'].'">Deactive</a></span>';
        ...
        <div class="lh-1 text-alternate">
            <a href="cat_edit.php?catid=<?php echo $data['cat_id']?>"> Edit </a>| <a href="cat_del.php?catid=<?php echo $data['cat_id']?>">Delete</a> </div>
        </div>
