...
if(isset($_POST['addbook'])){
    ...
    if(isset($_FILES['book'])){
        $errors = array();
        $filename = rand(0,10).$_FILES['book']['name'];
        ...
        move_uploaded_file($tmpname,'book/'.$filename);
        }
        else if(empty($errors) == false){
            echo "<script>alert('Please check the file format OR file size');</script>";
            echo "<script>window.location.href = 'http://localhost:82/ebook/admin/addbook.php';</script>";
            die();
    ...
    $ins_pro = "INSERT INTO book_stock(id,book,book_status) VALUES('{$id}','{$filename}','{$bstatus}')";
...
<div class="mb-3">
<label class="form-label">Book Stock ID</label>
...
<select class="select-single-no-search" name="bstatus">
...
<input type="submit" name="addbook" value="Add Book" class="btn btn-primary btn-outline-light">
