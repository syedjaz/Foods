...
if(isset($_POST['addbanner'])){
    ...
    if(isset($_FILES['banner_img'])){
        ...
        move_uploaded_file($tmpname,'media/'.$filename);
            ...
            echo "<script>window.location.href = 'http://localhost:82/ebook/admin/addbanner.php';</script>";
            die();
    ...
    $ins_pro = "INSERT INTO banner(banner_img,text1,text2,text3) VALUES('{$filename}','{$head1}','{$head2}','{$head3}')";

    $res = mysqli_query($con,$ins_pro);
    if($res){
    echo "<script>alert('Banner Info Inserted Successfully');</script>";
...
<span class="text-small align-middle">Banners</span>
</a>
...
<input type="text" class="form-control" name="head1">
</div>

<div class="mb-3">
<label class="form-label">Heading 2</label>
<input type="text" class="form-control" name="head2">
</div>


<div class="mb-3">
<label class="form-label">Heading 3</label>
<input type="text" class="form-control" name="head3">
</div>
...
<input type="submit" name="addbanner" value="Add Book" class="btn btn-primary btn-outline-light">
